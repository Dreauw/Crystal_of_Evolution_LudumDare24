package entities.towers {
	import entities.enemies.EnemyBase;
	import net.flashpunk.graphics.Image;
	import net.flashpunk.masks.Hitbox;
	import net.flashpunk.masks.Masklist;
	import utils.Resource;

	public class StrongTower2 extends TowerBase {
		public static var price:uint = 100;
		public function StrongTower2(x:Number, y:Number) {
			super(x, y);
			graphic = new Image(Resource.STRONG_TOWER_2);
			mask = getMask();
			orealX = (graphic as Image).width / 2;
			orealY = (graphic as Image).height / 2;
			reloadTime = 5;
			power = 15;
		}
		
		static public function getMask():Hitbox {
			return new Hitbox(22, 41, 0, 0);
		}
		
		static public function getDescription():String {
			return "Slow but huge dammage\nCan't shoot flying entity\nPrice : "+StrongTower2.price.toString()+"PO";
		}
	}
}