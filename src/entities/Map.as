package entities 
{
	...
	public class Map {
		...
		public function load(mapClass:Class):void {
			...
			world.add(cursor);
			world.addGraphic(Particle.instance, 0);
			world.add(new TowerSelector(cursor,(world as WorldGame).lvl));
			world.addGraphic((world as WorldGame).towerInformer, 0);
			world.addGraphic((world as WorldGame).text, 0);
			world.add((world as WorldGame).button);
			world.addGraphic((world as WorldGame).mobComing);
			...
			for each(var line:String in s) {
				...
				if (val[0].indexOf("event_layer") >= 0) {
					for each(var ev:String in val[1].split(",")) {
						...
						if (subVal[0] == "Crystal") crystal = entit[entit.length - 1] as Crystal;
					}
